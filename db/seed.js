...
const seedMembers = () => db.Promise.map([
  ...
  {firstName: 'Darth', middleName: null, lastName: 'Vader-Mitchell', ppid: 'Z666998', party: 'I', chamber: 'senate', state: 'NY', district: '69', electionYear: '2020'},
...
//   .then((issue) => issue.addIssue_bills(4))
//   .then(billIssue => billIssue.setDataValue('forOrAgainst', 'for'))
//   .then(() => Issue.findById(1))
//   .then((issue) => issue.addIssue_bills(5))
//   .then(billIssue => billIssue.setDataValue('forOrAgainst', 'against'))

// // .then(([bill, issue]) => bill.addIssue_bill(issue, {forOrAgainst: 'for'}))

  // .then(() => Promise.all([Issue.findById(1), Issue.findById(2)]))
  // .then((issues) => Promise.all([issues, Bill.findById(5)]))
  // .then(([issues, bill]) => bill.addIssue_bills(issues))
...
// tohere
  ...
  // .then(() => {
  //   return Promise.all([Bill.findOne({where: {prefix: 's', number: '84', session: '115'}}), Issue.findOne({where: {catCode: 'A3100'}})])
  // })
  // .then(([bill, issue]) => bill.addIssue_bill(issue, {forOrAgainst: 'for'}))
  ...
  // .then(() => Member.findById(1))
  // .then((member) => member.addBill_votes(4, {position: 'yes'}))
  ...
  .then(() => Member.findById(1))
  .then(member => member.getCatScore(1, 2012, 2016))
  .then(catScore => console.log('Lloyd Gun Control Cat Score is: ', catScore))
  // .then(() => Member.findById(1))
  // .then(member => member.getCatScore(1, 2012, 2016))
  // .then(score => console.log("Lloyd's array of scores for alignment to possible user positions on Gun Control are:\n", score, '\nIn order, these alignments are if the user\n [strongly disagrees, disagrees, is neutral, agrees, strongly agrees] with the issue.'))
